...
// Constructor Default
WorldState::WorldState() { currPosition = EnemyNone; }

// Get Enemy Position
EnemyPositions WorldState::getEnemyPosition() {
  // IrSensor Array = {Left, MidLeft, Mid, MidRight, Right}
  // Use for flag robots - "trick" represents double positions of enemy, common
  // for flag robots if (irStates[2]->getValue() == 1 && irStates[1]->getValue()
  // == 1) {
...
OnLine WorldState::getIsOnLine() {
  ...
  // Test reading from circle to get white and black values (0 is black and 1000
  // is white) Tested for circular platform ~950 for black and <300 for white
...
bool WorldState::getAllFrontActive() {
  return irStates[1]->getValue() == 1 && irStates[2]->getValue() == 1 &&
         irStates[3]->getValue() == 1;
}