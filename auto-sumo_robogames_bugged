...
int leftDir = 0;
int rightDir = 1;
int pLeftDir = leftDir;
int pRightDir = rightDir;
void setup() {
  Serial.begin(9600);
...
void move(int motor, int speed, int direction) {
  //motor: LEFT 2 RIGHT 1
  //speed: 0 is off, and 255 is full speed
  //direction: 0 FWD, 1 REV
  //^ double check conventions
  //pLeftDir and pRightDir used b/c esc safety logic
  ...
  if(motor == 2) {  //Left,   2
    if(pLeftDir != direction) {
      stop();
    }
    pLeftDir = direction;
    leftMotor.writeMicroseconds(ppmSpeed);
  } else if(motor == 1) {          //Right , 1
    if(pRightDir != direction) {
      stop();
    }
    pRightDir = direction;
...
void stop() {
  ...
  delay(350); //this is near the mininum delay required for a quick reverse
}
