...
{
    sf::RectangleShape foundation(sf::Vector2f(WINDOW_WIDTH, FULL_HEIGHT / 4));
    foundation.setFillColor(sf::Color(200, 200, 210));
    foundation.setPosition(0, FULL_HEIGHT * .80); // position it in the bottom left corner ideally
    window_->draw(foundation);

    sf::RectangleShape time_label(sf::Vector2f(WINDOW_WIDTH / 3, FULL_HEIGHT / 13));
    time_label.setFillColor(sf::Color(250, 250, 210));
    time_label.setPosition(10, FULL_HEIGHT * .82);
    ...
    timer.setPosition(15, FULL_HEIGHT * .84);
    window_->draw(timer);
}
...
{ // ./sim.sw (r1 x left of 0) (r1 y up of 0) (r1 angle in rad cw) (r2 x right of 0) (r2 y down of 0) (r2 angle in rad cw) (duration for sim: 0 = realtime elapsed)
    robot_texture.loadFromFile("simulator/res/robot_sprite.png");
    default_font.loadFromFile("simulator/res/arial.ttf");
    double r1_x, r1_y, r1_a, r2_x, r2_y, r2_a = 0.0;

    // reset button ideally goes back to here.
    if (argc == 8)
    {
    ...
    window_ = std::make_shared<sf::RenderWindow>(sf::VideoMode(WINDOW_WIDTH, FULL_HEIGHT), "My window");
    window_->clear(sf::Color::White); // clear the window with white color
